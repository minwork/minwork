...
class Factory
{
     ...
     *
     * @return DatabaseStorageInterface|MySqlTable|SqliteTable
    ...
    public static function getUserStorage(): DatabaseStorageInterface
    {
        if ($storage = self::getClassObject(self::USER_STORAGE)) {
            ...
            if (empty($storage->getFields())) {
                throw new DatabaseException("Database storage table does not exists, run install.php file to automatically create it");
            }
            return $storage;
        }
    }
